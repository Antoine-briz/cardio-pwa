  ...
  $app.innerHTML = `
    <section>
      ...
      <div class="grid">
        ...
        <button class="btn btn-anesth-important" onclick="location.hash = '#/anesthesie/consultations'">
  Consultations
</button>
...
const DATA = {
    "Endartériectomie carotidienne": {
      ...
      gestion:
        ...
        "Pré-commande&nbsp;: 2 CGR",
      monitorage: "Scope 5 branches, SpO2,  VVP, KTa (radial controlatéral), TOF, BIS, NIRS",
      alr:
    ...
    "Syndrome du défilé thoraco brachial (1°côte) ou STTB": {
      ...
      gestion:
        ...
        "- Arrêt AOD J-5<br><br>" +
        "Pré-commande&nbsp;: 2 CGR",
      monitorage: "Scope 5 branches, SpO2, VVP, PNI (controlatéral), TOF, BIS",
...
function renderInterventionEndoprotheses() {
  ...
  const DATA = {
    ...
    "Endoprothèse aortique thoracique (TEVAR)": {
      ...
      protocole: `<strong>Induction:</strong> Anesthésie générale AIVOC propofol/rémifentanil (Remplacé par : «&nbsp;Etomidate 0,3mg/kg car induction à risque&nbsp;» si induction à risque coché), +/- Atracurium 0,5mg/kg (Remplacé par: «&nbsp;Rocuronium 1,2mg/kg ou Célocurine 1mg/kg car séquence rapide&nbsp;» si séquence rapide coché)<strong>Antibioprophylaxie: </strong>Céfazoline 2g puis 1g toutes les 4h Si IMC &gt; 50 coché: Céfazoline 4g puis 2g toutes les 4h. Si allergie cochée: Vancomycine 30mg/kg IVL une injection 30min avant incision<br><br><strong>Hémostase: </strong>Héparine <strong>50 UI/kg</strong>, pas de monitorage de l’ACT. Antagonisation par Protamine en ratio 1/1 (100% de la dose d’HNF)<br><br><strong>Objectif tensionnel:</strong><br>- PAM 80-90mmHg hors déploiement car risque d'ischémie médullaire<br>- Déploiement en zone 0-1: Pacing rapide 180/min pour PAM 40-50mmHg<br>- Déploiement en zone 2-3: Clévidipine QSP PAS 80-100mmHg<br><br><strong>ALR:</strong> Pas d’ALR`,
      alr: `Si abord scarpa: <br>Carré des lombes + Bloc fémoral + Bloc ilio-inguinal ilio-hypogastrique <br>&nbsp;Cf QLB 🖥️&nbsp;&nbsp;Cf bloc fémoral 🖥️ Cf BIIIH 🖥️<br><br>Si abord huméral: <br>Bloc supraclaviculaire <br>Cf supra-claviculaire 🖥️<br>`,
    ...
    "Endoprothèse aortique fenêtrée (TEVAR)": {
      ...
      protocole: `<strong>Induction:</strong> Anesthésie générale AIVOC propofol/rémifentanil (Remplacé par : «&nbsp;Etomidate 0,3mg/kg car induction à risque&nbsp;» si induction à risque coché), +/- Atracurium 0,5mg/kg (Remplacé par: «&nbsp;Rocuronium 1,2mg/kg ou Célocurine 1mg/kg car séquence rapide&nbsp;» si séquence rapide coché)<strong>Antibioprophylaxie: </strong>Céfazoline 2g puis 1g toutes les 4h Si IMC &gt; 50 coché: Céfazoline 4g puis 2g toutes les 4h. Si allergie cochée: Vancomycine 30mg/kg IVL une injection 30min avant incision<br><br><strong>Hémostase: </strong>Héparine <strong>50 UI/kg</strong>, objectif d'ACT 250-300s. Antagonisation par Protamine en ratio 1/1 si &lt; 2h (½ dose 2-4h, 0 &gt; 4h)<br><br><strong>Objectif tensionnel:</strong><br>- PAM 80-90mmHg hors déploiement car risque d'ischémie médullaire<br>- Déploiement en zone 0-1: Pacing rapide 180/min pour PAM 40-50mmHg<br>- Déploiement en zone 2-3: Clévidipine QSP PAS 80-100mmHg<br><br><strong>ALR:</strong> Pas d’ALR`,
      alr: `Si abord scarpa: <br>Carré des lombes + Bloc fémoral + Bloc ilio-inguinal ilio-hypogastrique <br>&nbsp;Cf QLB 🖥️&nbsp;&nbsp;Cf bloc fémoral 🖥️ Cf BIIIH 🖥️<br><br>Si abord huméral: <br>Bloc supraclaviculaire <br>Cf supra-claviculaire 🖥️<br>`,
      orientation: `SSPI 24h<br><br>Examens:&nbsp;&nbsp;<br>- ECG + bilan complet à l’admission<br>- ECG + bilan complet à H+2<br><br>Surveillance:&nbsp;&nbsp;<br>- Neurologique (déficit médullaire)<br>- Ischémie MI<br>- Saignement<br><br>Objectifs:&nbsp;&nbsp;<br>Hb &gt; 10g/dL, PAM 80 à 90mmHg, température, capnie, SaO2, Gly, Na`
    },
...
/* ===========================
   ETO (tableau 3 colonnes) - Pontages
   =========================== */
...
function initEtoEntryButtons(prefix) {
  document.getElementById(`${prefix}-eto-goto`)?.addEventListener("click", () => {
    location.hash = "#/reanimation/eto";
  });
  document.getElementById(`${prefix}-eto-open`)?.addEventListener("click", () => {
    openEtoFormModal(prefix);
  });
}

// --- Petit helper : lien cliquable vers une image (nom affiché) ---
function etoImgLink(label, imgFile) {
  if (!imgFile) return "";
  return `<a href="#" class="eto-imglink" onclick="openImg('${imgFile}');return false;" title="Voir l'image">${label}</a>`;
}

// --- Formulaire ETO (dans une modal) : TABLEAU 3 colonnes + conditions au-dessus ---
function etoFormHtmlCompact(prefix) {
  return `
       ...
       <tbody>
        ...
        <thead>
          <tr>
            ...
            <th class="col-img">Coupes / mesures (cliquer)</th>
          </tr>
        </thead>
...
function etoFormHtmlCompactPlastieMitrale(prefix) {
  ...
  // Remplace UNIQUEMENT le <tr> "Valve mitrale" du formulaire général
  const mitraleRegex = /<tr>\s*<td class="eto-sec">Valve mitrale<\/td>[\s\S]*?<\/tr>/;

  const plastieMitraleRow = `
<tr>
  <td class="eto-sec">Valve mitrale</td>
  ...
  <td>
    <div class="eto-cell">
      ...
      <label>Centrage IM
        <select id="${prefix}-eto-im-centrage">
          <option value="">—</option>
          <option>Centrée</option>
          <option>Excentrée</option>
        </select>
  ...
  // Si jamais la regex ne matche pas, on retourne le général (mais ça évite un crash silencieux)
  if (!mitraleRegex.test(base)) return base;
...
function openEtoSynthese(text) {
  ...
  overlay.innerHTML = `
    <div class="acr-modal-card" role="dialog" aria-modal="true">
      ...
      <pre class="acr-modal-body" id="eto-synth-text" style="white-space:pre-wrap;">${(text || "—")}</pre>
      <div class="acr-modal-actions" style="display:flex; gap:10px; justify-content:flex-end;">
        <button class="btn" id="eto-copy">Copier</button>
  ...
  overlay.querySelector("#eto-copy")?.addEventListener("click", async () => {
    const t = overlay.querySelector("#eto-synth-text")?.textContent || "";
    try {
      await navigator.clipboard.writeText(t);
    } catch (e) {
      const ta = document.createElement("textarea");
  ...
  const encadres = [
    ...
    },
    {
      ...
      html: `
        ...
        <p>
          Solumédrol 120 mg IVL à l'induction (en plus des 4 mg/kg pré-opératoire) + 120mg IVL en sortie de CEC
        </p>
      `,
    },
    {
  ...
  return `
    <section class="eto-section">
      ...
      <div class="eto-block" style="display:none;">
        <ul class="eto-list">
          <li>
            Fonction systolique VG post-transplantation
            <span class="eto-icon" onclick="openImg('transplantVG.png')">🖥️</span>
          </li>
          <li>
            Fonction VD post-transplantation
            <span class="eto-icon" onclick="openImg('transplantVD.png')">🖥️</span>
          </li>
          <li>
            Recherche de bulles post-transplantation
            <span class="eto-icon" onclick="openImg('transplantbulles.png')">🖥️</span>
          </li>
          <li>
...
//  - Page protocoles = SPLIT (gauche sélection + droite résultat live)
// =====================================================================
...
// -------------------------------------------------
// Markup SAFE : on garde escapeHtml, puis on réinjecte nos tokens
...
// -------------------------------------------------
// 6) Calculs / Recettes (remplace consignes orange)
// -------------------------------------------------
function cecNum(x){
...
const CEC_PPT_TABLES = {
  ...
  video_thoraco: [
    ...
    [
      "Anticoagulation",
      "Bolus d’héparine 300 UI/kg IVD dans dans l'OD (ACT cible: > 400 sec) \nSi résistance à l'héparine, discuter AT3 (Cf résistance à l'héparine) \nSi TIH: Bivalirudine 1mg/kg bolus puis 2,5 mg/kg/h (ACT cible: 300-400 sec) \nProtamine: 60-80% de la dose d’héparine (ACT cible: < 120 sec) \nHemostase post-CEC: Selon Quantra (Cf algorithme Quantra) ",
      ""
    ],
    [
      "Cardioplégie\nCf cardioplégies",
  ...
  dissection: [
    ...
    ["Anticoagulation", "Bolus d’héparine 300 UI/kg IVD dans l'OD (ACT cible: > 400 sec) \nSi résistance à l'héparine, discuter AT3 (Cf résistance à l'héparine) \nSi TIH: Bivalirudine 1mg/kg bolus puis 2,5 mg/kg/h (ACT cible: 300-400 sec) \nProtamine: 60-80% de la dose d’héparine (ACT cible: < 120 sec) \nHemostase post-CEC: Selon Quantra (Cf algorithme Quantra) ", "", ""],
...
function toggleDissectionLine(){
  ...
  bindChk("cec-ia", s.coeur, "ia");
  bindChk("cec-stenose", s.coeur, "stenoseCoronaireSerree");
...
function renderCecUrgencesMenu() {
  $app.innerHTML = `
    <section class="intervention-shell cec-urgences-shell">
...
`;
  ...
  const renderTable = () => {
    ...
    // sélection lignes (Ctrl / Cmd multi)
    ...
    // bouton ouvrir (sans changer la sélection)
...
// helper: détecte smartphone (tu peux ajuster le seuil)
function isMobile() {
  return window.matchMedia("(max-width: 900px)").matches;
}

/* =========================
   PAGE BIBLIOGRAPHIE (responsive)
